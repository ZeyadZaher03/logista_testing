...
db.ref(`users/${uid}/tasks`).on("value", function (tasks) {
	...
	tasks.forEach((taskData) => {
		...
		// const
		const taskItemHTML = `<div data-task_id="${
			task.taskUid
		}" class="map_info-col__item map_info-col__item-task">
		...
		document.querySelector(
			".map_info-col__subhead-tasks[data-tasktab='0'] span"
		).innerHTML = numOfUnAssigned;
		document.querySelector(".map_info-col__subhead-tasks[data-tasktab='1'] span").innerHTML = numOfAssigned;
		document.querySelector(
			".map_info-col__subhead-tasks[data-tasktab='2'] span"
		).innerHTML = numOfCompleted;
	});
...
db.ref(`users/${uid}/drivers`).on("value", function (driversData) {
	...
	const inActiveAgentTabSpanContainer = document.querySelector(
		".map_info-col__subhead-item[data-agentTab='2'] span"
	);
	...
	const onChangeAssignTask = (assignTaskToDriverInput, driverTeam, driverId) => {
		let pastValue;
		const taskId = assignTaskToDriverInput.options[assignTaskToDriverInput.selectedIndex].value;
		db.ref(`users/${uid}/drivers/${driverTeam}/${driverId}/tasks`)
			.once("value", (snapshot) => (pastValue = snapshot.val()))
			.then(() => {
				let newValue;
				if (pastValue !== null) {
					newValue = [...pastValue, taskId];
				} else {
					newValue = [taskId];
				}
				...
				db.ref(`users/${uid}/tasks/${taskId}`).update({
					driverId,
					status: 0,
	...
	driversData.forEach((drivers) => {
		drivers.forEach((snapshot) => {
			...
			const DriverContainerColoumn = document.querySelector(
				`.map_info-col__containar-tabAgnet[data-agentTab='${driverItemTabIndex(
					driversStatus
				)}']`
			);
				...
				db.ref(`users/${uid}/drivers/${driverItemTeam}/${driverItemId}`).on(
					...
					(snapshot) => {
						...
						db.ref(`users/${uid}/tasks/${activeTaskId}`)
							.once("value", (snapshot) => {
								if (activeTaskId !== 0) {
									return (activeDriverTaskName =
										snapshot.val().pickup.address.name +
										" " +
										snapshot.val().pickup.address.name);
								}
							})
                      ...
                      <div class="taskDetails_content">
                        ...
                        <button class="deleteDetails" data-driver_team=${driverTeamValue} data-driver_id=${driverId} onclick="deleteDriverRun()"id="deleteDriver">Delete</button>
								...
								const addTaskToDriverSelect = document.querySelector(
									"#changeTask"
								);
								...
								db.ref(`users/${uid}/tasks`).on("value", (snapshot) => {
									...
									snapshot.forEach((taskData) => {
										...
										const pickupPlaceName = taskData.val()
											.pickup.address.name;
										const deliverPlaceName = taskData.val()
											.pickup.address.name;
										if (taskData.val().driverId == 0) {
